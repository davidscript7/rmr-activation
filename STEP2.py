...
COL_SALES_DOC_ITM = "Sales Document Items"
...
def load_step1_data():
    ...
    # Fallback for older .step1_data.json without date_label
    if not date_label:
        date_label = data.get("processing_date")
    if not date_label:
        from datetime import datetime
        date_label = datetime.now().strftime("%B")

    if sheets:
        label = ", ".join(sheets) if isinstance(sheets, list) else str(sheets)
        print(f"  📅 Sheets processed in STEP1: {label}")

    print(f"  🏷️  Date label: {date_label}")
...
def save_intermediate(df, date_label):
    mapping = {}
    ...
    with open(INTER_STEP2, "w", encoding="utf-8") as f:
        json.dump({"key_to_cfin": mapping, "date_label": date_label}, f, indent=2)
...
def main():
    ...
    print(f"     Name: CONTRACT DETAIL {date_label}.xlsx")
    print()
